...
def main():
    ...
    while current_iter <= total_iters:
        ...
        while train_data is not None:
            ...
            if current_iter % opt['logger']['print_freq'] == 0:
                ...
                if opt.get("wandb", False) and wandb.run:
                    ...
                    wandb.log(wandb_iter_payload, step=current_iter)
        ...
        if opt.get('val') is not None and val_loader is not None: # Ensure val_loader exists
            # Optionally, only run validation if it's not the very start (current_iter > 0)
            # or if a certain number of iterations have passed, to avoid validating with an untrained model too early.
            # This is a design choice. For simplicity, we run it every epoch completion.
            logger.info(f"--- Running validation at end of Epoch {epoch} (current_iter: {current_iter}) ---")
            ...
            if opt.get("wandb", False) and wandb.run and validation_results:
                ...
                wandb.log(wandb_val_payload, step=current_iter) # Log with current_iter as step
                logger.info(f"Logged validation results to wandb at end of Epoch {epoch}.")
    ...
    if opt.get('val') is not None and val_loader is not None: # Final validation
        ...
        if opt.get("wandb", False) and wandb.run and final_val_results:
            ...
            wandb.log(wandb_final_val_payload, step=current_iter)
