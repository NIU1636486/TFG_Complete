...
parser.add_argument("--docum", action="store_true", help="Use document dataset", default=False)
...
match args.model_type:
    case "unet":
        if args.pretrained:
            unet = U_Net(output_ch=1)

        else:
            unet = U_Net().to(device)
    case "r2attunet":
        if args.pretrained:
            unet = R2AttU_Net(output_ch=3)

        else:
            unet = R2AttU_Net().to(device)
    case "attunet":
        if args.pretrained:
            unet = AttU_Net(output_ch=1)
...
for epoch in range(num_epochs):
    ...
    with torch.no_grad():
        ...
        for val_batch_idx, (images, GT )in enumerate(valid_loader):
            images, GT = images.to(device), GT.to(device)
