...
film_dataset = FilmDataset(dataset_root, limit=None, seed=42, export_csv=True, csv_path=args.output_name+"_"+args.model_type+"_"+str(num_epochs)+"epochs_dataset_map.csv", new_method=args.nou_loader, sigma=1, white=0.3)

generator = torch.Generator().manual_seed(25)

train_dataset, test_dataset = random_split(film_dataset, [0.6, 0.4], generator=generator)
test_dataset, val_dataset = random_split(test_dataset, [0.1, 0.9], generator=generator)
...
for epoch in range(num_epochs):
    ...
    for images, GT in train_loader:
        ...
        gt_np_bin = (GT[0].detach().cpu().numpy() > 0.2).astype(np.uint8)
        sr_np_bin = (SR_probs[0].detach().cpu().numpy() > 0.2).astype(np.uint8)
    ...
    if WANDB:
        wandb.log({"epoch": epoch, "train_loss": epoch_loss, "train_dice": train_dice_total, "train_iou": train_iou_total, "train_acc": acc, 
        "images_train": [
    ...
    with torch.no_grad():
        for images, GT in valid_loader:
            ...
            pred_bin = (SR > 0.5).astype(np.uint8)
            target_bin = (GT > 0.5).astype(np.uint8)
            dices, ious = [], []
            for p, t in zip(pred_bin, target_bin):
                dices.append(dice_coef(t, p))
                ious.append(iou(t, p))
            val_dice_total, val_iou_total = np.mean(dices), np.mean(ious)
    ...
    if WANDB:
        wandb.log({ "val_acc": acc, "val_loss": val_loss, "epoch": epoch, "val_dice": val_dice_total, "val_iou": val_iou_total,
        "images_val": [
        wandb.Image(images[0].cpu().detach().numpy().transpose(1, 2, 0)),
        wandb.Image(final_image),
...
model = unet.to("cpu")
torch.save(model.state_dict(), unet_path.split(".")[0]+"_final.pth")
