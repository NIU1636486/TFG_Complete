...
import cv2 as cv
from damage_generator.sample import sample_num_artifacts, sample_size_artifacts, sample_closest_in_area
...
def create_random_mask_with_transparency(target_size, df, df_synthetic, df_counts,
                       use_synthetic=False, 
                       min_artifacts=5, max_artifacts=100, rescale=True, 
                       uniform_sample=False, verbose=True, alpha_min=0.5, alpha_max=1.0, alpha_prob=0.1, damage_level=1.0):                  
    if rescale:
        rescale_factor = target_size[0]/2560 if target_size[0] <= target_size[1] else target_size[1]/2560
    else:
        rescale_factor = 1.
    ...
    for _, artifact_row in selected_artifacts_df.iterrows():
        try:
            ...
            if np.random.rand() < alpha_prob:  # Apply transparency with probability alpha_prob
                alpha = np.random.uniform(alpha_min, alpha_max)  # Random transparency
                mask_final[x1:x2, y1:y2] = (1 - alpha) * mask_final[x1:x2, y1:y2] + alpha * artifact
            else:
                mask_final[x1:x2, y1:y2] = np.where(artifact > mask_final[x1:x2, y1:y2], artifact, mask_final[x1:x2, y1:y2])    
            # mask_final[x1:x2,y1:y2] = np.where(artifact>mask_final[x1:x2,y1:y2], artifact, mask_final[x1:x2,y1:y2])
            i=i+1
